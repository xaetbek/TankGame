...
import main.java.tankrotationexample.Bullet;
import java.awt.*;
...
public class Tank extends GameObjects {
    ...
    void setGW(Launcher gw_to_set) { //this is needed for spawning bullets
        this.gw = gw_to_set;
    }
    ...
    private void checkBorder() {
        ...
        if (x >= Launcher.WORLD_WIDTH - 88) {
            x = Launcher.WORLD_WIDTH - 88;
        }
        if (y < 40) {
            y = 40;
        }
        if (y >= Launcher.WORLD_HEIGHT - 80) {
            y = Launcher.WORLD_HEIGHT - 80;
        }
    }


    private void SpawnBullet(int x, int y, int vx, int vy, int angle, Launcher gw) { //private access because only the tank is able to spawn bullets
        Bullet blt = new Bullet(x, y+20, angle);
    ...
    public void drawImage(Graphics2D graph) {
