...
class CollisionHandler {
    ...
    ArrayList<GameObjects> HandleCollisions(ArrayList<GameObjects> game_objects) {

        for (int i = 0; i < game_objects.size(); i++) {

            for (int j = i; j < game_objects.size(); j++) {
                GameObjects obj_at_i = game_objects.get(i);
                GameObjects obj_at_j = game_objects.get(j);
                ...
                if (i != j) {
                    ...
                    if (obj_at_i instanceof Bullet && obj_at_j instanceof Tank && !(((Bullet) obj_at_i).getOwner().equals(((Tank) obj_at_j).getTag())) && !((Bullet) obj_at_i).collided) { // we make sure the bullet hasn't already collided(so we don't call collisions again)
                        if (obj_at_i.bounds.intersects(obj_at_j.bounds)) {
                            obj_at_i.collision();
                    ...
                    if (((obj_at_j instanceof Bullet && obj_at_i instanceof main.java.tankrotationexample.BreakableWall && !((Bullet) obj_at_j).collided))) {
                        if (obj_at_i.bounds.intersects(obj_at_j.bounds)) {
                            obj_at_j.collision();
                            obj_at_i.collision();
                        }
                    ...
                    if (obj_at_i instanceof Tank && obj_at_j instanceof main.java.tankrotationexample.BreakableWall) {
                        Rectangle r1 = ((Tank) obj_at_i).getOffsetBounds();
                        if (r1.intersects(obj_at_j.bounds)) {
                    ...
                    if (obj_at_i instanceof main.java.tankrotationexample.BreakableWall && obj_at_j instanceof Tank) {

                        Rectangle r2 = ((Tank) obj_at_j).getOffsetBounds();
                        if (r2.intersects(obj_at_i.bounds)) {  //intersection occurs
