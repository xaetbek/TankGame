...
public class Bullet extends GameObjects {
    ...
    Bullet(int x, int y, int angle) {
        ...
        this.angle = angle;
        this.bounds = new Rectangle(x, y, bullet_img.getWidth(), bullet_img.getHeight());
    }

    public void update() {
        if (!collided) {  //if it's not collided, keep moving, else don't move the bounds
            this.x = x + vx;
            this.y = y + vy;
    ...
    private void checkBorder() { //this allows us to mark the bullet as inactive(so it can later be removed) if out of map bounds
        ...
        int right_limit = Launcher.WORLD_WIDTH - 65;
        if (x >= right_limit) {
            this.isInActive = true;
        }
        int lower_limit = 40;
        ...
        int upper_limit = Launcher.WORLD_HEIGHT - 60;
        if (y >= upper_limit) {
            this.isInActive = true;
        }
    }
